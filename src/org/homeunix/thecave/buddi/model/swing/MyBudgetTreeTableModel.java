...
public class MyBudgetTreeTableModel extends AbstractTreeTableModel {
	...
	public Object getValueAt(Object node, int column) {
		...
		if (node instanceof BudgetCategory){
			...
			if (column >= 1 && column < getColumnCount()){
				...
				retValues[0] = bc;
				retValues[1] = bc.getBudgetPeriodContainingDate(getColumnDate(column));
				...
				//long value = bc.getBudgetPeriodContainingDate(getColumnDate(column));
	...
	private long getChildTotal(Object node, int column){
		if (PrefsModel.getInstance().isShowFlatBudget())
			return ((BudgetCategory) node).getBudgetPeriodContainingDate(getColumnDate(column));
		if (node instanceof BudgetCategory){
			long childTotal = ((BudgetCategory) node).getBudgetPeriodContainingDate(getColumnDate(column));
