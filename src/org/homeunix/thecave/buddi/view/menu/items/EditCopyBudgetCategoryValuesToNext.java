...
public class EditCopyBudgetCategoryValuesToNext extends MossMenuItem{
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (BudgetCategory bc : ((MainFrame) getFrame()).getBudgetCategoriesInSelectedPeriod()) {
			long amount = bc.getBudgetPeriodContainingDate(currentlySelectedDate);
			try {
				if (bc.getBudgetPeriodContainingDate(bc.getBudgetPeriodType().getBudgetPeriodOffset(currentlySelectedDate, 1)) == 0)
