				...
				if (!t.isDeleted()){
					if (t.getFrom().equals(associatedSource)){
						if (t.isClearedFrom()){
							...
							notClearedTotal -= t.getAmount(); //(t.getBudgetPeriodContainingDate() * (t.isInflow() ? 1 : -1));
						...
						if (t.isReconciledFrom()){
							...
							notReconciledTotal -= t.getAmount(); //(t.getBudgetPeriodContainingDate() * (t.isInflow() ? 1 : -1));
					}
						...
						for (TransactionSplit split : t.getFromSplits()){
							if (split.getSource().equals(associatedSource)){
								if (t.isClearedFrom()){
									...
									notClearedTotal -= split.getAmount(); //(t.getBudgetPeriodContainingDate() * (t.isInflow() ? 1 : -1));
								...
								if (t.isReconciledFrom()){
									...
									notReconciledTotal -= split.getAmount(); //(t.getBudgetPeriodContainingDate() * (t.isInflow() ? 1 : -1));
